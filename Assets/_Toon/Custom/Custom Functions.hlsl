...
void  GetShadow_float(float2 uv, out float Shadow)
{
    Shadow = 1;

    #ifndef SHADERGRAPH_PREVIEW
        //Shadow = SAMPLE_TEXTURE2D_X_LOD(_CachedShadowmapAtlas, s_linear_clamp_sampler, uv, 0.0);
    #endif 
}

void Sample_float(float2 uv, out float3 Sample)
{
    Sample = 1;

    #ifndef SHADERGRAPH_PREVIEW
       //Sample = SAMPL E_TEXTURE2D_X_LOD(_AmbientOcclusionTexture, s_linear_clamp_sampler, uv, 0.0);
       Sample = SAMPLE_TEXTURE2D_X_LOD(_ScreenSpaceShadowsTexture, s_linear_clamp_sampler, uv, 0.0);
       //PositionInputs posInput;
       //posInput.positionSS = uv;
       //Sample = GetScreenSpaceShadow(posInput, 1);
    #endif
}
